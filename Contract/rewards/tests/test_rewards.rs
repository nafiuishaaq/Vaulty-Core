...
fn test_rewards_pool_initialization() {
    ...
    let mut client = rewards::RewardsContractClient::new(&env, &contract_id);
    ...
    let pool = client.get_rewards_pool();
...
fn test_add_custom_milestone() {
    ...
    admin.require_auth();
    client.add_milestone(&180, &100_0000000);

    let milestones = client.get_milestones();
    // 4 default + 1 custom = 5 milestones
    assert_eq!(milestones.len(), 5);
    assert_eq!(milestones[4].streak_threshold, 180);
    assert_eq!(milestones[4].reward_amount, 100_0000000);
}
